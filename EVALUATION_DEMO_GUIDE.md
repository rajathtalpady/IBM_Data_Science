...
**What to say:**
> "I have **50 backend test cases** across 12 test files, covering every layer of the application — infrastructure, auth, and all four CRUD operations on employees."
...
| `test_employees_get.py` | 6 | 200 list, one returned after POST, correct fields, multiple returned, deleted not returned, reflects PUT updates |
| `test_employees_put.py` | 9 | 200 updated field, single field update, all fields at once, DB persistence, unchanged fields not overwritten, 404 unknown ID, `updated_at` auto-set, 422 empty body, 403 non-admin |
| `test_employees_delete.py` | 9 | 204 success, removed from DB, GET returns 404 after delete, only target removed, 404 unknown ID, idempotent second delete, 422 malformed ID, 422 whitespace ID, empty response body |
...
| Operation | Where used | Code |
|-----------|-----------|------|
| `insert_one` | Create employee / Register user | `db.employees.insert_one(employee)` |
| `find_one` | Get by ID / Login lookup | `db.employees.find_one({"employee_id": id})` |
| `find` | Get all employees | `db.employees.find()` — async cursor iteration |
| `update_one` with `$set` | Update employee fields | `db.employees.update_one({"employee_id": id}, {"$set": data})` |
| `delete_one` | Delete employee | `db.employees.delete_one({"employee_id": id})` |
| `update_one` with `$push` | Append activity log entry | `db.users.update_one({"email": e}, {"$push": {"activity_log": entry}})` |
| `create_index` | Enforce uniqueness at DB level | `db.employees.create_index(["employee_id"], unique=True)` |
...
**What to say:**
> "I've implemented **6 design patterns** in the React frontend."
